...
export function ServerConnectionPanel({snapshot,notify,mode=client.mode}:{snapshot:Snapshot;notify:(message:string)=>void;mode?:'mock'|'remote'}){
  ...
  useEffect(()=>{setHost(saved.host);setPortText(String(saved.port));setVersion(saved.version)},[saved.host,saved.port,saved.version,saved.revision]);
  ...
  return <div className="panel server-connection"><h2>Server Connection</h2><p className="muted">{remote?'全Botが停止中のみ保存できます。次のStartから新しい接続先を使います。':'Save は次回接続から適用します。MockではMinecraftに接続しません。'}</p>
    {remote&&<p className="mono">Current: {saved.host}:{saved.port} · {saved.version}</p>}
    ...
    <div className="server-actions"><button className="button outline" disabled={!next||busy||remoteActive} onClick={()=>void save(false)}>Save</button>{!remote&&<button className="button accent" disabled={!next||busy} onClick={()=>{setError('');setConfirm(true)}}>Save &amp; Reconnect</button>}</div>
