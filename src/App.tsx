...
function App(){
  ...
  return <div className="app-shell">
    ...
    <main className="content" id="main"><div className="desktop-top"><div className="breadcrumb">WORKSPACE <ChevronRight size={14}/> {current.label.toUpperCase()}</div><div className="top-right"><span className="version-pill">JAVA EDITION <b>1.8.9</b></span><span className="mode-pill"><span className="mode-dot"/> {client.mode==='remote'?'REMOTE MODE':'MOCK MODE'}</span></div></div>
      ...
      {client.mode==='remote'&&page==='jobs'&&<div className="insight"><CircleHelp size={18}/><p>この画面の編集はMockモード用です。実Botの操作はBotsページから行ってください。</p></div>}
      ...
      {page==='jobs'&&client.mode==='mock'&&<section className="view-section"><div className="job-actions"><label>Mock event for <select value={jobInstance} onChange={e=>setJobInstance(e.target.value)}>{snapshot.instances.map(i=><option key={i.id} value={i.id}>{i.id}</option>)}</select></label><button className="button accent" onClick={()=>perform(()=>client.createJob(jobInstance),'Mock Jobを追加しました')}><Plus size={17}/> Create job</button></div>
