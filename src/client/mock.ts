import { botStates, type AccountKind, type BBotClient, type Bot, type BotState, type InstanceStatus, type JobStatus, type Settings, type Snapshot } from './types';
...
const initial = ():Snapshot => ({
  ...
  settings:{maxBots:20,pathConcurrency:2,eventPollingSeconds:10,debug:false,javaVersion:'1.8.9'},revision:0
});
...
export class MockBBotClient implements BBotClient {
  ...
  stopBot(id:string) {this.update(s=>{const b=this.getBot(s,id);if(b.state==='DISCONNECTED')return;this.leave(s,b);b.state='DISCONNECTED';b.updatedAt=Date.now();this.log(s,`${b.name} を停止`, 'INFO',{botId:id});});}
  recoverBot(id:string) {this.update(s=>{const b=this.getBot(s,id);if(b.state==='DISCONNECTED')return;this.leave(s,b);b.state='RECOVERING';b.updatedAt=Date.now();this.log(s,`${b.name} を復旧状態に変更`, 'WARN',{botId:id});});}
  ...
  reset(){this.sequence=100;this.snapshot=initial();this.listeners.forEach(fn=>fn());}
}
